  ...
  const bearishStocks = useMemo(() => {
    return FO_SYMBOLS
      .map((sym) => {
        ...
        const proximity = Math.abs(stock.open - stock.high) / stock.open * 100;
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-destructive/10 border border-destructive/30">
          ...
          <span className="text-destructive font-bold text-sm">Open = High → Bearish Mode</span>
        </div>
        <span className="text-xs text-muted-foreground">{bearishStocks.length} stocks qualifying</span>
      </div>

      {/* Explanation */}
      <div className="glass-card rounded-xl p-4 border border-destructive/20 bg-destructive/5">
        <div className="flex items-start gap-3">
          <Zap className="w-4 h-4 text-destructive mt-0.5 flex-shrink-0" />
          <div>
            <p className="text-sm font-semibold text-destructive mb-1">Bearish Signal Detected</p>
            <p className="text-xs text-muted-foreground">
              When a stock opens at or near its day high, it signals that sellers dominated from the very start of the session.
              This indicates strong downward pressure and potential for continued selling throughout the day.
              Stocks shown here have Open within <strong className="text-destructive">{PROXIMITY_THRESHOLD}%</strong> of their High.
            </p>
          </div>
        </div>
      </div>
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {bearishStocks.length === 0 ? (
                ...
                bearishStocks.map(({ symbol, stock, proximity }) => {
                  ...
                  return (
                    <tr key={symbol} className="border-b border-border/50 hover:bg-surface/30 transition-colors">
                      ...
                      <td className="px-3 py-2 text-right">
                        <span className="px-2 py-0.5 rounded-full bg-destructive/20 text-destructive text-xs font-bold border border-destructive/30">
                          BEARISH
                        </span>
                      </td>
                      ...
                      <td className="px-3 py-2 text-right font-mono text-success">{stock.high.toFixed(2)}</td>
                      <td className="px-3 py-2 text-right font-mono text-destructive font-bold">{proximity.toFixed(3)}%</td>
